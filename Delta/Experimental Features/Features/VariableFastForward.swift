...
struct FastForwardSpeed: RawRepresentable {
    ...
    static func speeds(in range: ClosedRange<Double>) -> [FastForwardSpeed] {
        ...
        var speeds = stride(from: range.lowerBound, to: range.upperBound, by: 1.0).dropFirst().map {
            FastForwardSpeed(rawValue: $0)
        }
...
extension FastForwardSpeed: CustomStringConvertible, LocalizedOptionValue {
    var description: String {
        if #available(iOS 15, *) {
            let formattedText = self.rawValue.formatted(
...
struct VariableFastForwardOptions {
    ...
    @Option(
        name: "Nintendo", description: "Preferred NES fast forward speed.",
        values: FastForwardSpeed.speeds(in: System.nes.deltaCore.supportedRates))
    var nes: FastForwardSpeed?

    @Option(
        name: "Super Nintendo", description: "Preferred SNES fast forward speed.",
        values: FastForwardSpeed.speeds(in: System.snes.deltaCore.supportedRates))
    var snes: FastForwardSpeed?

    @Option(
        name: "Sega Genesis", description: "Preferred Genesis fast forward speed.",
        values: FastForwardSpeed.speeds(in: System.genesis.deltaCore.supportedRates))
    var genesis: FastForwardSpeed?

    // @Option(name: "Nintendo 64", description: "Preferred N64 fast forward speed.", values: FastForwardSpeed.speeds(in: System.n64.deltaCore.supportedRates))  // Temporarily disabled
    // var n64: FastForwardSpeed?  // Temporarily disabled

    @Option(
        name: "Game Boy Color", description: "Preferred GBC fast forward speed.",
        values: FastForwardSpeed.speeds(in: System.gbc.deltaCore.supportedRates))
    var gbc: FastForwardSpeed?

    @Option(
        name: "Game Boy Advance", description: "Preferred GBA fast forward speed.",
        values: FastForwardSpeed.speeds(in: System.gba.deltaCore.supportedRates))
    var gba: FastForwardSpeed?

    @Option(
        name: "Nintendo DS", description: "Preferred DS fast forward speed.",
        values: FastForwardSpeed.speeds(in: System.ds.deltaCore.supportedRates))
    var ds: FastForwardSpeed?

    @Option(
        name: "Allow Unrestricted Speeds",
        description:
            "Allow choosing speeds that exceed the maximum supported speed of a system.\n\nThis can be used to test the performance of new iOS devices."
    )
...
extension Feature where Options == VariableFastForwardOptions {
    subscript(gameType: GameType) -> FastForwardSpeed? {
        get {
            ...
            // case .n64: return self.n64  // Temporarily disabled
        ...
        set {
            ...
            // case .n64: self.n64 = newValue  // Temporarily disabled
