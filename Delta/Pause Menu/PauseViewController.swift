...
class PauseViewController: UIViewController, PauseInfoProviding
{
    ...
    var pauseItems: [MenuItem] {
        return [self.saveStateItem, self.loadStateItem, self.cheatCodesItem, self.fastForwardItem, self.sustainButtonsItem, self.screenshotItem, self.restartItem].compactMap { $0 }
    }
