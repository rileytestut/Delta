...
extension Game {
    public typealias Setting = __GameSetting

    public static let melonDSBIOSIdentifier = "com.rileytestut.MelonDSDeltaCore.BIOS"
    public static let melonDSDSiBIOSIdentifier = "com.rileytestut.MelonDSDeltaCore.DSiBIOS"
}
...
public class Game: _Game, GameProtocol {
    ...
    public override var artworkURL: URL? {
        get {
            ...
            if let unwrappedArtworkURL = artworkURL {
                if unwrappedArtworkURL.isFileURL {
                    ...
                    artworkURL = URL(
                        fileURLWithPath: unwrappedArtworkURL.relativePath,
                        relativeTo: DatabaseManager.gamesDirectoryURL)
                } else if let host = unwrappedArtworkURL.host?.lowercased(),
                    host == "img.gamefaqs.net" || host == "gamefaqs1.cbsistatic.com",
                    var components = URLComponents(
                        url: unwrappedArtworkURL, resolvingAgainstBaseURL: false)
                {
        ...
        set {
            ...
            if let newValue = newValue, newValue.isFileURL {
                ...
                artworkURL = URL(
                    fileURLWithPath: newValue.lastPathComponent,
                    relativeTo: DatabaseManager.gamesDirectoryURL)
            }
    ...
    var settings: [Setting: Any] {
        get {
            ...
            var settings = gameSettings.map { (Setting(rawValue: $0), $1) }.reduce(into: [:]) {
                $0[$1.0] = $1.1
            }
    ...
    private var defaultSettings: [Setting: Any] {
        ...
        // if let internalName, internalName.contains("DONKEY KONG 64"), self.type == .n64  // Temporarily disabled
        // {
    ...
    var internalName: String? {
        // guard self.type == .n64 else { return nil }  // Temporarily disabled for NFC MVP
        return nil  // N64 functionality disabled
        ...
        do {
            guard let fileHandle = FileHandle(forReadingAtPath: self.fileURL.path) else {
                return nil
            }
            ...
            _internalName = String(data: data, encoding: .utf8)?.trimmingCharacters(
                in: .whitespacesAndNewlines)
            return _internalName
        } catch {
            Logger.main.error(
                "Failed to read internal ROM name: \(error.localizedDescription, privacy: .public)")
            return nil
        }
    }
...
extension Game {
    class var recentlyPlayedFetchRequest: NSFetchRequest<Game> {
        ...
        fetchRequest.sortDescriptors = [
            NSSortDescriptor(keyPath: \Game.playedDate, ascending: false),
            NSSortDescriptor(keyPath: \Game.name, ascending: true),
        ]
...
extension Game {
    override public func prepareForDeletion() {
        ...
        guard
            !managedObjectContext.insertedObjects.contains(where: {
                ($0 as? Game)?.identifier == self.identifier
            })
        else { return }

        // Double-check fileURL is NOT actually a directory, which we should never delete.
        var isDirectory: ObjCBool = false
        guard FileManager.default.fileExists(atPath: self.fileURL.path, isDirectory: &isDirectory),
...
extension Game: Syncable {
    ...
    public var syncableFiles: Set<File> {
        ...
        case Game.melonDSBIOSIdentifier:
            let bios7File = File(
                identifier: "bios7", fileURL: MelonDSEmulatorBridge.shared.bios7URL)
            let bios9File = File(
                identifier: "bios9", fileURL: MelonDSEmulatorBridge.shared.bios9URL)
            let firmwareFile = File(
                identifier: "firmware", fileURL: MelonDSEmulatorBridge.shared.firmwareURL)

            return [artworkFile, bios7File, bios9File, firmwareFile]

        case Game.melonDSDSiBIOSIdentifier:
            let bios7File = File(
                identifier: "bios7", fileURL: MelonDSEmulatorBridge.shared.dsiBIOS7URL)
            let bios9File = File(
                identifier: "bios9", fileURL: MelonDSEmulatorBridge.shared.dsiBIOS9URL)
            let firmwareFile = File(
                identifier: "firmware", fileURL: MelonDSEmulatorBridge.shared.dsiFirmwareURL)
    ...
    public func awakeFromSync(_ record: AnyRecord) throws {
        guard let gameCollection = self.gameCollection else {
            throw SyncValidationError.incorrectGameCollection(nil)
        }
