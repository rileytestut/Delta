...
class PreviewGameViewController: DeltaCore.GameViewController {
    ...
    private var emulatorCoreQueue = DispatchQueue(
        label: "com.rileytestut.Delta.PreviewGameViewController.emulatorCoreQueue",
        qos: .userInitiated)
    ...
    override var game: GameProtocol? {
        ...
        didSet {
            ...
            emulatorCore.addObserver(
                self, forKeyPath: #keyPath(EmulatorCore.state), options: [.old],
                context: &kvoContext)

            let size = CGSize(
                width: emulatorCore.preferredRenderingSize.width * 2.0,
                height: emulatorCore.preferredRenderingSize.height * 2.0)
...
extension PreviewGameViewController {
    ...
    fileprivate func preparePreview() {
        ...
        if let saveState = self.previewSaveState as? SaveState {
            saveState.managedObjectContext?.performAndWait {
                previewSaveState = DeltaCore.SaveState(
                    fileURL: saveState.fileURL, gameType: saveState.gameType)
            }
        }
    ...
    fileprivate func copySaveFiles() {
        ...
        for fileURL in fileURLs {
            do {
                let destinationURL = self.temporaryDirectoryURL.appendingPathComponent(
                    fileURL.lastPathComponent)
                try FileManager.default.copyItem(
                    at: fileURL, to: destinationURL, shouldReplace: true)
    ...
    fileprivate func restoreSaveFiles(removeCopyDirectory: Bool) {
...
extension PreviewGameViewController: GameViewControllerDelegate {
    ...
    func gameViewController(
        ...
        // N64-specific function - temporarily disabled for NFC MVP
        ...
        //     return [:]
        // }
        return [:]  // Return empty options since N64 is disabled
    }
}
