...
extension AchievementsTracker {
    static let didUnlockAchievementNotification = Notification.Name(
        "DLTADidUnlockAchievementNotification")
    ...
    internal init(emulatorCore: EmulatorCore, authenticatedClient: OpaquePointer) throws {
        ...
        guard let authUserInfo = rc_client_get_user_info(authenticatedClient) else {
            throw AchievementsError(
                errorCode: AchievementsError.notAuthenticated,
                message: NSLocalizedString("User is not logged in.", comment: ""))
        }

        self.client = rc_client_create(
            { address, buffer, numberOfBytes, client in
                AchievementsManager.read(
                    address: address, buffer: buffer, numberOfBytes: numberOfBytes, client: client)
            },
            { request, callback, callbackData, client in
                AchievementsManager.send(
                    request: request, callback: callback, callbackData: callbackData, client: client
                )
            })
        ...
        rc_client_enable_logging(self.client, Int32(RC_CLIENT_LOG_LEVEL_VERBOSE)) {
            message, client in
...
extension AchievementsTracker {
    func start() async throws {
        ...
        let consoleType =
            switch System(gameType: self.emulatorCore.deltaCore.gameType) {
            case .nes: RC_CONSOLE_NINTENDO
            case .snes: RC_CONSOLE_SUPER_NINTENDO
            // case .n64: RC_CONSOLE_NINTENDO_64  // Temporarily disabled for NFC MVP
            case .gbc, .gba, .ds, .genesis, nil:
                throw AchievementsError(
                    errorCode: AchievementsError.unsupportedSystem,
                    message: NSLocalizedString("System is not yet supported.", comment: ""))
            }

        let callback:
            @convention(c) (Int32, UnsafePointer<CChar>?, OpaquePointer?, UnsafeMutableRawPointer?)
                -> Void = { (result, errorMessage, client, userData) in
                    guard
                        let userData = userData?.assumingMemoryBound(
                            to: AchievementsManager.UserData.self
                        ).pointee, let continuation = userData.continuation
                    else { return }
                    ...
                    if result == RC_OK {
                        ...
                        continuation.resume(
                            throwing: AchievementsError(
                                errorCode: Int(result), message: errorMessage))
                    }
                }
        ...
        try await withCheckedThrowingContinuation { continuation in
            ...
            rc_client_begin_identify_and_load_game(
                self.client, UInt32(consoleType), (self.gameURL as NSURL).fileSystemRepresentation,
                nil, 0, callback, userData)
        }
...
extension AchievementsTracker {
    func process(_ achievement: Achievement) {
        NotificationCenter.default.post(
            name: AchievementsTracker.didUnlockAchievementNotification, object: self,
            userInfo: [AchievementsTracker.achievementUserInfoKey: achievement])
    }

    func readBytes(at address: Int, into buffer: UnsafeMutablePointer<UInt8>, size: Int) -> Int {
        guard
            let data = self.emulatorCore.deltaCore.emulatorBridge.readMemory?(
                at: address, size: size)
        else { return 0 }
...
extension AchievementsTracker {
    fileprivate func didRenderFrame() {
